...
Number of taxa in trees in Open Tree Of Life database (one per study) and number of taxa on Abadi et al. (2019). The median number of taxa in the Abadi et al. empirical trees were `r round(median(empirical_summary$ntax, na.rm=TRUE),1)` (95% CI: `r round(quantile(empirical_summary$ntax, probs=c(0.025, .975),na.rm=TRUE),1)`), while the median number in Open Tree of Life were `r round(median(otol_summary$ntax, na.rm=TRUE),1)` (95% CI: `r round(quantile(otol_summary$ntax, probs=c(0.025, .975),na.rm=TRUE),1)`).
